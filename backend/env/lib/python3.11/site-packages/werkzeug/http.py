...
def quote_header_value(value: t.Any, allow_token: bool = True) -> str:
    r"""Add double quotes around a header value. If the header contains only ASCII token
...
def unquote_header_value(value: str) -> str:
    r"""Remove double quotes and decode slash-escaped ``"`` and ``\\`` characters in a
    header value.
...
def parse_options_header(value: str | None) -> tuple[str, dict[str, str]]:
    r"""Parse a header that consists of a value with ``key=value`` parameters separated
